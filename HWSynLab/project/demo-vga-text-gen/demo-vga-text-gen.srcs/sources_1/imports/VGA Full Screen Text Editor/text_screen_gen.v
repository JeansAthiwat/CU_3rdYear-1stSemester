...
module text_screen_gen(
    ...
    // new cursor position
    assign cur_x_next = (set && cur_x_reg == 7) ? 0 :               // reset x to 0 after 8 chars
                        (set) ? cur_x_reg + 1 :                     // move right after symbol placement
                        (move_xr_tick && cur_x_reg < MAX_X - 1) ? cur_x_reg + 1 :   // manual right move
                        (move_xl_tick && cur_x_reg > 0) ? cur_x_reg - 1 :           // manual left move
                        cur_x_reg;                                  // no move
    
    assign cur_y_next = (set && cur_x_reg == 7 && cur_y_reg < MAX_Y - 1) ? cur_y_reg + 1 :   // move to next row after 8 chars
                        (move_yu_tick && cur_y_reg > 0) ? cur_y_reg - 1 :                    // manual up move
                        (move_yd_tick && cur_y_reg < MAX_Y - 1) ? cur_y_reg + 1 :            // manual down move
                        cur_y_reg;                                                           // no move
